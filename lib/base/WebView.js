	...
	var  WebView = function WebView(pathObj, renderQ)  {
		...
		this.compose = function compose(isMain)  {
			...
			fs.readdir( webView.viewPath, function(err, files) {
                if (err)  {
                    renderQ.getItem(true).receiver(util.format('<div class="_xsError">Missing the block view directory for the %s block.</div>', getRsOpPath(webView.viewPath)));
                    renderQ.seal();
                }
				else  {
	...
	WebView.prototype.composeView = function composeView(dynaNode, cb)  {
        ...
        async.series([
            ...
            function composing(callback)  {
            	composeBlockView(webView, dynaNode, callback);
            }
        ],
...
function  composeBlockView(webView, dynaNode, cb)  {
