...
var  WebView = (function()  {
	...
	var  WebView = function(renderOpt, renderQ)  {
        ...
        let  cssID = renderQ.getCssID();
        if (!cssID)
            renderQ.setClazName( autoGenClazName(this, renderOpt) );

		this.compose = function(uic, pfix, cb)  {
            let  blockRoot = this.getRootPath() + '/blocks/views',
                 uicPath = this.getOpPath(),
                 headers = renderQ.srvObj['http-headers'],
                 isPage = renderQ.srvObj.isPage;
            
            if (uic)  {
                cssID = cssID || (isPage  ?  'mainBlk' : genAutoID());

                emulator.runXS(uic, cssID, uicPath, blockRoot, renderQ).then( xsElm => {
                    let  isReload = headers['x-xs-reload'],
                         cssPrefix = '.' + blkUtil.pathToClazName(renderOpt.opPath);

                    try  {
                        let  isFrag = !renderQ.asPage() && !renderQ.isExt;
                        if (isFrag)  {
                            // generates a HTML-fragment
                            let  htmlFG = '<script type="text/javascript">\n<% js(); %></script>\n' + xsElm.toHTML();
                            if (!isReload)
                                htmlFG = '<style type="text/css">\n<% css(); %></style>\n' + htmlFG;

                            let  dynaNodeMock = {html: htmlFG};
                            fillInHTML(dynaNodeMock, renderQ);  // this will inherintly call renderQ.add()
                        }
                        else
                            renderQ.add( xsElm.toHTML() );

                        // the top level top is default to '_mainBlk'
                        let  js = xsElm.toJS(isReload),
                             blkID = renderQ.getCssID() && isPage  ?  'mainBlk' : xsElm.id;

                        js = 'let _' + blkID + '=__._' + blkID + '=' + js;
                        if (!headers['x-xs-blockid'])
                             js += '\n__.addToInit("' + blkID + '");';

                        /*
                        if (!renderQ.getCssID() && isPage)
                            js = 'let _mainBlk=__._mainBlk=' + js;
                        else  if (isFrag)
                            js = 'let _' + xsElm.id + '=__._' + xsElm.id + '=' + js;
                        */
                        ...
                        let  rtnObj = {
                            body: hasj.reportError('view', util.format('Block [%s] failed to create its view.', uicPath))
                                      .add('pre', err.stack)
                                      .toHTML()
                        };
                        cb( false, rtnObj );
                ...
                .catch ( (e) => {
                    console.log( e.stack );
                    let  rtnObj = {
                        body: hasj.reportError('view', util.format('Failed to build block [%s].', uicPath))
                                  .add('pre', e.stack)
                                  .toHTML()
                    };
                    cb( false, rtnObj );
    ...
    function  handleBlockFiles(webView, cb)  {
        fs.readdir( webView.getViewPath(), function(err, files) {
            if (err)  {
                let  rtnObj = {
                        body: util.format('<div class="_xsErr">Block [%s] missing the view directory.</div>', webView.getOpPath())
                    };
                cb( false, rtnObj );
            }
            else  {
                let  dynaNode = prepareDynaNode( webView, files );
