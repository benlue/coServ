		...
		this.compose = function compose(isMain)  {
			...
			fs.readdir( viewPath, function(err, files) {
				if (err === null)  {
					...
					dynaNode.locale = renderQ.getSite().locale;
	...
	WebView.prototype.composeView = function composeView(dynaNode)  {
		...
		if (dynaNode.jsF)
			try  {
				...
				renderQ.addJS( js, this.clazName );
			}
			catch (e) {}
...
// TODO: a brute force approach. need a better parser...
/*
function  getParameter(fn)  {
	var  args = [],
		 i = fn.lastIndexOf(')');
	if (i > 0)
		fn = fn.substring(0, i).trim();
	...
	while (fn.length > 0)  {
		...
		if (idx === -1)  {
			...
			if (idx === -1)  {
				idx = fn.indexOf('"');
				idx2 = fn.indexOf('"', idx+1);
			}
			else
				idx2 = fn.indexOf('\'', idx+1);
		}
		else  {
			var  arg1 = getParameter( fn.substring(0, idx) );
			if (arg1.length > 0)
				args = args.concat(arg1);
			
			idx2 = fn.indexOf('}', idx+1) + 1;
			idx--;
		}
		
		//if (idx >= 0)  {
			args.push( fn.substring(idx+1, idx2).trim() );
			
			var  idx3 = fn.indexOf(',', idx2);
			fn = idx3 >= 0  ?  fn.substring(idx3+1).trim() : '';
		//}
		//else
		//	fn = '';
	}
