...
function  loadDynaNode(webView, dynaNode, cb)  {
    ...
    async.parallel([
        ...
        function loadXS(cb)  {
            if (dynaNode.xs)
                fs.readFile( dynaNode.xs, {encoding: 'utf8'}, function(err, xs)  {
                    ...
                    if (xs)  {
                        ...
                        if (renderQ.parentQ)
                            dynaNode.html = '<div id="' + cssID + '" ' + classAttr + '></div>';
                        else  {
                            if (!renderQ.asPage() && !renderQ.isExt)
                                // generates a HTML-fragment
                                dynaNode.html = wrapHTMLFrag(cssID, classAttr, '');
                        }

                        // then, the javascript
                        let  cssPrefix = renderQ.getClazName(true),
                             parentID = renderQ.pCssID  ?  '"' + renderQ.pCssID + '"' : 'null',
                             jsConclude = "ctrl.init = function() {let code=exports.make();" + 
                                          util.format("__.setCSS( xs.toCSS(code.css, 0, '%s') );", cssPrefix) + 
                                          util.format("$(ctrl.getTarget()).html( code.html.toHTML(0, '%s') );};", cssID),
                             jsPrelude = wrapJsCode(cssID, webView.getOpPath(), parentID, xs, jsConclude),
                             js = fillInAux( jsPrelude, dynaNode, renderQ );
                        renderQ.addJS( js );
                    }
                    cb();
                });
...
function  wrapJsCode(cssID, opURL, parentID, jsData, jsConclude)  {
    let  js = util.format("(function(exports)  {\n\tvar  ctrl = new _ctrl('#%s', '%s');\n", cssID, opURL);
    js += util.format("\tvar  _parent = %s;\n\tctrl.getParent = function() {return _parent  ?  __.getCtrl(_parent) : null;}\n%s\n%s\nreturn ctrl;\n})({});", parentID, jsData, jsConclude);
