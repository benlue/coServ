...
const  async = require('async'),
       ...
       emulator = require('../wcomp/EmulateUIC'),
...
function  loadDynaNode(webView, dynaNode, cb)  {
    ...
    async.parallel([
        ...
        function loadXS(cb)  {
            if (dynaNode.xs)  {
                let  blockRoot = webView.getRootPath() + '/blocks/views/',
                     uicPath = webView.getOpPath();

                let  xsElm = emulator.runXS(cssID, uicPath, blockRoot),
                     cssPrefix = renderQ.getClazName(true);

                dynaNode.html = xsElm.toHTML();
