...
var  WebView = (function()  {
    ...
    /**
     * noWrap: do not generate the wrapping <div>.
     */
	var  WebView = function(pathObj, renderQ, noWrap)  {
		this.rootPath = pathObj.rootPath;
		this.viewPath = pathObj.path + '/';
        this.opPath = pathObj.opPath.replace(/\\/g, '/');
        this.noWrap = noWrap;

        // tell SASS where to find @import files
        this.sassBase = [path.join(this.rootPath, './sass'), path.join( this.rootPath, '../../cont/sass')];
		...
		this.compose = function(isMain, cb)  {
			var  webView = this;
