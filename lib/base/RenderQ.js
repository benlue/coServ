...
var  RenderQ = (function()  {
	...
	var  RenderQ = function RenderQ(webSite, srvObj)  {
		if (arguments.length == 1)  {
			parentQ = arguments[0];

			this.parentQ = parentQ;
			this.pCssID = parentQ.cssID;
			this.webSite = parentQ.webSite;
		}
		else  {
									...
									switch (item.type)  {
										...
										case 'incJs':
											if (rq.incJS)  {
												...
												//lines += '<script type="text/javascript" src="/coServ.js"></script>\n';
												lines += '<script type="text/javascript" src="/xs.js"></script>\n';
											}
											break;
									}
									htmlStr += lines;
		...
		return  (function(htmlList) {
			var  value = rq.srcData.value || {},
				 ...
				 ctx = {
				 	...
				 	locID: mapLocaleToID(rq.getLocale()), pageURI: rq.getPageURI(),
					caCode: rq.getSite().siteInfo.caCode, themeRoot: rq.getSite().themeRoot
				 },
				 */
				 bi = rq.srvObj,
				 _params = rq.compParams,
	...
	RenderQ.prototype.setRootPath = function(rp)  {
		this.rootPath = rp;
	};

	RenderQ.prototype.getRootPath = function()  {
	...
	*/
	...
	RenderQ.prototype.addInc = function addInc(inc)  {
		if (this.parentQ)
			...
			for (var i in lines)  {
				...
				if (c === '/' && line.charAt(1) !== '/')  {
					var  themeRoot = this.webSite.themeRoot;
					
					line = '/' + themeRoot + '/cont' + line;
				}
				else  {
				...
				if (!fileType)  {
					...
					if (line.indexOf('.css', llen - 4) !== -1)
						...
						// exclude 'xs.js' from the include. will force to /lib/client/xs.js
						if (line.slice(-6) !== '/xs.js')
						//if (line.indexOf('xs.js', llen - 9) === -1)
							fileType = 'js';
					}
				}
	...
	RenderQ.prototype.createCtx = function(id)  {
		return  createXsBlockCtx(id, this);
	}

	RenderQ.prototype.invokeBlock = function(id, srvURI, params, html)  {
		let  bkThis = {
				rq: this,
				html: html
			 };

		blockFun.call(bkThis, id, srvURI, params);
	}

	RenderQ.prototype.invokeUIC = function(id, srvURI, params, html)  {
		let  bkThis = {
				rq: this,
				html: html
			 };

		wcompFun.call(bkThis, id, srvURI, params);
	}


	RenderQ.prototype.buildChildQ = function(id, params, srvURI)  {
		let  subRQ = new RenderQ( this );
	
		if (id)
			subRQ.setCssID( id );
		//subRQ.setSrcData( this.getSrcData() );
	
		let  webSite = this.getSite(),
			 ...
			 cmReq = {
				...
				//themePath: webSite.getRootPath(client.category),
				client: client,
				locale: this.srvObj.locale
			 };

		subRQ.srvObj = webSite.verifyURI(cmReq);
		return  subRQ;
	}
...
function  blockFun(id, srvURI, params, option)  {
	...
	if (srvURI === '')  {
		...
		let  opURI = rq.getPageURI(),
			 rootPath = rq.getRootPath(),
			 vPath = path.join( rootPath, 'blocks/views' + opURI);

		subRQ.srvObj = rq.srvObj;
		this.html.push( new Promise(function(resolve, reject) {
			new WebView( {rootPath: rootPath, path:vPath, opPath:opURI}, subRQ ).compose(rq.uic, 'hf', function(isStream, result)  {
				resolve( result.body );
			} );
		}));
	}
...
function  wcompFun(id, srvURI, params, options)  {
	...
	let  opURI = srvURI,
		 ...
		 vPath = path.join( rootPath, wcompPath + srvURI);
	...
	this.html.push( new Promise(function(resolve, reject) {
		new WebView( pathObj, subRQ )
			.compose(null, 'hf', function(isStream, result)  {
				resolve( result.body );
			} );
	}));
}
...
function  printFun(htmlStr)  {
	this.html.push( htmlStr );
}
...
/**
 * Created a context for old-styled blocks.
 * @param {*} rq 
 */
function  createBlockCtx(rq)  {
	let  ctx = {
			id: rq.cssID || '', 
			pCssID: rq.pCssID,
			...
			pageURI: rq.getPageURI(),
			caCode: rq.getSite().siteInfo.caCode , 
			themeRoot: rq.getSite().themeRoot
		 };
...
/**
 * Created a context for XS blocks. Note that "ctx" and "bi" have been merged to a context object.
 * @param {*} rq 
 */
function  createXsBlockCtx(id, rq)  {
	let  bi = rq.srvObj,
		 ctx = {
			id: id,
			uri: bi.uri.replace(/\\/g, "/"),
			endpID: bi.hasOwnProperty('id')  ? bi.id : null,
			query: bi.query,
			title: rq.getPageTitle(), 
			description: rq.getPageDescription(),
			postfix: bi.postFix,
			appCode: rq.getSite().siteInfo.caCode,
			//'http-headers': bi['http-headers'],
			cookies: bi.cookies,
			locale: rq.getLocale(),
			client: bi.client
		 };
