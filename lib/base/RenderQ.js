...
var  RenderQ = (function()  {
		...
		this.seal = function(cb)  {
			...
			for (var i in queue)  {
				...
				if (item.hasOwnProperty('code'))  {
					...
					if (idx2 > 0)  {
						if (prog.indexOf('block(') === 0 || prog.indexOf('wcomp(') === 0 ||
							prog.indexOf('region(') === 0)
							jsProg += "\nsetIdx(" + i + ");";
					}
			...
			Promise.all(queue).then(function()  {
				...
				for (var i in queue)  {
					var  item = queue[i];
					//console.log('item[%d]:\n%s', i, JSON.stringify(item, null, 4));
	...
	*/
	...
	RenderQ.prototype.setClazName = function setClazName(cname)  {
		if (cname)
			this.clazName = '.' + cname.replace(/\\/gi, '');
	};
...
function  wcompFun(id, srvURI, params)  {
	if (arguments.length === 1)  {
		srvURI = id;
		id = null;
	}
	...
	//params = params || {};
	// 'knowAs' should be deprecated
	//id = id || params.knownAs;
	...
	console.log('wcomp path is ' + wcompPath);
	...
	this.getQueue()[qIdx] = new Promise(function(resolve, reject) {
		new WebView( pathObj, subRQ )
			.compose( false, function(isStream, result)  {
				resolve( result.body );
			} );
	});
}
