...
var  RenderQ = (function()  {
	...
	*/
	...
	RenderQ.prototype.buildModel = function(id, params, srvURI, rq, cb)  {
		let  subRQ = new RenderQ( rq );
		...
		let  webSite = rq.getSite(),
			 ...
			 srvObj = webSite.verifyURI(cmReq),
			 resObj = rq.getResponseObj();
		subRQ.srvObj = srvObj;
	
		cmReq.inData._loc = cmReq.inData._loc || rq.getLocale();
		
		webSite.buildBlockModel(cmReq, srvObj, resObj, (result) => {
			cb(result, subRQ);
		});
	}
...
function  blockFun(id, srvURI, params, option)  {
	...
	if (srvURI === '')  {
		...
		this.html.push( new Promise(function(resolve, reject) {
			new WebView( {rootPath: rootPath, path:vPath, opPath:opURI}, subRQ ).compose('hf', function(isStream, result)  {
				resolve( result.body );
			} );
		}));
	}
