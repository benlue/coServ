...
var  RenderItem = (function()  {
	...
	RenderItem.prototype.receiver = function(html)  {
		...
		this.seal = function()  {
			...
			if (pendingCount === 0)  {
				if (this.incCssItem)  {
					...
					this.incCssItem.receiver( lines );
				}
				if (this.incJsItem)  {
					var  lines = '';
					if (this.incJS)
					...
					this.incJsItem.receiver( lines );
				}
				if (this.cssItem)
					this.cssItem.receiver( this.css || '');
				if (this.jsItem)
					this.jsItem.receiver( this.js || '');
...
function  blockFun(srvURI, option)  {
	...
	if (srvURI === '')  {
		...
		this.getSite().runBlock(null, cmReq, subRQ);
	}
};
