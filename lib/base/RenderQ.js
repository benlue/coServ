		...
		this.seal = function()  {
			...
			if (pendingCount === 0)  {
				if (this.incCssItem)  {
					var  lines = '';
					if (this.incCSS)
						for (var i = 0, len = this.incCSS.length; i < len; i++)
				...
				if (this.incJsItem)  {
					var  lines = '';
					if (this.incJS)
						for (var i = 0, len = this.incJS.length; i < len; i++)
				...
				(function(hline) {
				    var  value = rq.srcData.value,
				    	 ...
				    	 ph = rq.phrases,
				    	 ctx = {locID: mapLocaleToID(rq.getSite().locale), pCssID: rq.pCssID,
...
function  blockFun(srvURI, option)  {
	...
	if (srvURI === '')  {
        ...
        console.log('will be runing block: %s', srvURI);
...
function  regionFun(rgn, options)  {
	...
	if (rgn[0] === '.')  {
		...
		var  idx = vpath.lastIndexOf(path.sep + 'views' + path.sep);	// both region and block paths starts from /views/
		opPath = 'rgn' + vpath.substring(idx+7);
	}
	else  {
	...
	new WebView( {rootPath: dynaNode.rootPath, path:vpath, opPath:opPath}, subRQ ).compose();
};
