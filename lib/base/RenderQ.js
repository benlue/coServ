		...
		if (this.parentQ)
			...
			for (var i in lines)  {
				...
				if (c === '/' && line.charAt(1) !== '/')  {
					...
					/*
					var  themeRoot = this.webSite.siteInfo.sitePath;
					if (themeRoot)  {
						var  idx = themeRoot.lastIndexOf('/');
						themeRoot = themeRoot.substring(idx+1);
					}
					else
						themeRoot = this.webSite.siteInfo.caCode;
					*/
					var  themeRoot = this.webSite.themeRoot;
					//console.log('theme path: %s, theme root: %s', this.webSite.siteInfo.sitePath, themeRoot);
...
function  blockFun(id, srvURI, params, option)  {
	...
	if (srvURI === '')  {
		...
		this.html.push( new Promise(function(resolve, reject) {
			new WebView( {rootPath: dynaNode.rootPath, path:vPath, opPath:opURI}, subRQ ).compose('hf', function(isStream, result)  {
				resolve( result.body );
			} );
		}));
	}
