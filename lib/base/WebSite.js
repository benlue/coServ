...
var  WebSite = (function()  {
	...
	WebSite.prototype.runBlock = function runBlock(res, cmReq, renderQ, cb) {
		if (cmReq.pathname[cmReq.pathname.length-1] === '/')
			cmReq.pathname = cmReq.pathname.substring(0, cmReq.pathname.length-1);
...
function  makeSrvObj(uriList, layoutMap, cmReq, needID)  {
	...
	if (isFormatSupported(postFix) && uriList.hasOwnProperty(pathname))  {
		...
		//console.log('block meta data:\n%s', JSON.stringify(uri, null, 4));
		...
		if (uri.id === needID || uri.id === 'maybe')  {
			...
			//console.log('webSite: block parameters: \n%s', JSON.stringify(inData, null, 4));

			if (isOk)  {
				// ok, this is a valid path. return its attributes
				var  isPage = postFix === 'html',
					 knownAs = inData  ?  inData._cs_knownAs : null;
				...
				srvObj = {
					...
					title: uri.title, description: uri.description
				};
