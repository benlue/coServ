...
var  WebSite = (function()  {
	...
	var  WebSite = function  WebSite(siteInfo)  {
		...
		let  themePath = path.join(themeBase, './themes/');
	...
	WebSite.prototype.buildBlockModel = function(cmReq, renderQ, res, cb) {
		...
		if (srvObj.service)  {
			...
			if (srvObj.isXS)  {
				let  isProtected = this.siteInfo.isProtected,
					 blockRoot = themePath + '/blocks',
					...
					if (uic.err)  {
						...
						if (uic.checkIn && (message = emulator.checkInput( uic.checkIn, cmReq.inData )))
							...
							emulator.buildModel(uic, cmReq.inData, ctx).then( result => {
								if (isProtected && result.token)
									// store token here
									setTokenAsCookie(res, result.token.token);
	...
	WebSite.prototype.runBlock = function(res, cmReq, renderQ, cb) {
		...
		if (srvObj.isValid && srvObj.isAuthorized)  {
			if (renderQ)
...
function packRequestInfo(webSite, cmReq, uriList)  {
	...
	if (webSite.authMethod)
		cmReq.isAuthorized = webSite.authMethod( cmReq.cookies.token );
 ...
 */
function setTokenAsCookie(res, token)  {
	// TODO: 'maxAge' should be configurable
	let  maxAge = 5184000000;	// set the token to be expired after 60 days.
	if (token)
