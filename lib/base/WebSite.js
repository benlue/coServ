...
var  WebSite = (function()  {
		...
		if (psURI.substring(0, 10) === '/_api/post')  {
			...
			let  userAgent = req.headers['user-agent'],
				 ...
				 cmReq = {
					...
					locale: locale,
					client: client,
					themePath: this.getRootPath(client.category)
				 };
	...
	WebSite.prototype.verifyURI = function(cmReq) {
		...
		return  packRequestInfo(this.siteInfo, cmReq, uriList);
	}
	 ...
	 * @param {*} cb 
	 */
	WebSite.prototype.buildBlockModel = function(cmReq, srvObj, res, cb) {
		if (srvObj.service)  {
			let  postData = {
					post: srvObj.query,
					url: srvObj.service,
			...
			if (mod)  {
				...
				try  {
					mod.execute(ctx, cmReq.inData, cb);
				}
				catch (e)  {
					console.log( e.stack );
					cb( {errCode: -1, message: 'Faild to run model.', value: e.stack});
				}
			}
			else
				cb( null );
	...
	WebSite.prototype.runBlock = function(res, cmReq, renderQ, cb) {
		...
		if (srvObj.isValid)  {
			...
			// 2. process the block
			this.buildBlockModel( cmReq, srvObj, res, (result) => {
				if (result)
					postModel(renderQ, result, cb);
				else
					viewIt(renderQ, {errCode: 0, message: 'No-op', value: cmReq.inData}, cb);
			});
		}
        else  {
        	var  rtnObj = {
 ...
 * @param {*} cb 
 */
function  postModel(renderQ, result, cb) {
 ...
 */
function packRequestInfo(siteInfo, cmReq, uriList)  {
	let  uriParts = cmReq.uri.split('/'),
	...
	if (!srvObj.isValid)  {
		...
		let  xsPath = path.join(cmReq.themePath, 'blocks/views/' + po.p + '.xs');
	...
	if (!srvObj.isValid)  {
		// the uri may has the 'id' section. remove the 'id' section and try the match again
		...
		if (srvObj.isValid)  {
			srvObj.id = id;
			cmReq.uri = uriParts.slice(0, -1).join('/');
		}
	}
...
function viewIt(renderQ, data, cb)  {
	var  srvObj = renderQ.srvObj,
		 httpHeaders = srvObj['http-headers'];

	let  blkID = httpHeaders['x-xs-blockid'] || srvObj.knownAs;
	if (blkID)
