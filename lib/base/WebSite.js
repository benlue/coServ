...
var  WebSite = (function()  {
	...
	WebSite.prototype.runBlock = function(res, cmReq, renderQ, cb) {
		...
		if (cmReq.uri.slice(-1) === '/')
			cmReq.uri = cmReq.uri.slice(0, -1);

		// 1. we need to know if this is a valid url.
		let  uriList = this.getUriList(cmReq.client.category),
			 srvObj = packRequestInfo(this.siteInfo, cmReq, uriList);
				...
				if (mod)  {
                    ...
                    try  {
                    	mod.execute(ctx, cmReq.inData, (result) => {
							postModule(ctx, renderQ, result, cb);
						});
                    }
                    catch (e)  {
                    	console.log( e.stack );
 ...
 * @param {*} cb 
 */
function  postModule(ctx, renderQ, result, cb) {
