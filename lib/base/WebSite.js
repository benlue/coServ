	...
	WebSite.prototype.runBlock = function(res, cmReq, renderQ, cb) {
		...
		if (srvObj.intPath)  {
			...
			if (renderQ)
				...
				// should set layout map to the top level
				var  layoutMap = this.getLayoutMap(clientCategory);
				renderQ.setLayoutMap( layoutMap );
	...
	if (isPage || pfix === 'hf')  {
		...
		var  opPath = srvObj.intPath,
             ...
             pathObj = {
             	...
             	path: rootPath + (isPage  ?  '/layout/' + mapLayout(renderQ.getLayoutMap(), opPath) : '/blocks/views' + opPath),
             	opPath: opPath
             },
