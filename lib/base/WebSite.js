 ...
 */
var  dd = require('../server/pp/DeviceDetector.js'),
	 fs = require('fs'),
	 path = require('path'),
	 url = require('url'),
...
var  WebSite = (function()  {
	var  WebSite = function  WebSite(siteInfo, rPath)  {
		...
		if (siteInfo.theme)  {
			if (typeof siteInfo.theme === 'string')
				rootPath.desktop = rPath + siteInfo.theme;
			else  {
				for (var k in siteInfo.theme)
					rootPath[k] = siteInfo.theme[k];
				this.supportMultiD = true;
			}
		}
		else
			rootPath.desktop = rPath + siteInfo.caCode;
	...
	WebSite.prototype.run = function run(req, res) {
		...
		if (psURI.substring(0, 10) === '/_api/post')  {
			...
			var  caCode = inData.caCode || (inData.hasCA  ?  '' : this.siteName),
				 apiPath = '/' + caCode + inData.url;
			// inData.hasCA should be deprecated. we kept it for backward compatibility
			if (inData.hasCA)
				caCode = inData.url.split('/')[0];
			...
			if (req.cookies.token)  {
				...
				inData.post._key = this.siteInfo.appKeys[caCode];
			...
			var  cmReq = {pathname: psURI, cookies: req.cookies, inData: inData,
						  headers: req.headers, client: dd.detect(req.headers['user-agent'])};
	...
	WebSite.prototype.viewIt = function viewIt(renderQ, srvObj, data)  {
		...
		if (pfix == 'wsj')
			...
			var  opPath = srvObj.intPath,
                 rootPath = this.getRootPath( srvObj.client.category ),
	...
	WebSite.prototype.isValid = function isValid(cmReq)  {
		...
		var  uriList = this.getUriList(cmReq.client.category),
			 layoutMap = this.getLayoutMap(cmReq.client.category),
		...
		if (uri.id === needID || uri.id === 'maybe')  {
			...
			if (isOk)  {
				...
				srvObj = {uri: cmReq.uri, query: inData, cookies: cmReq.cookies, 'http-headers': cmReq.headers,
						  intPath: pathname, args: uri.args, postFix: postFix, service: uri.service,
