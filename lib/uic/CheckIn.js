...
exports.verifyObjectInput = function(rules, key, inObj)  {
    ...
    Object.getOwnPropertyNames(rules).some( p => {
        ...
        if (p == '_cb' && (typeof rule == 'function'))
            // the final callback
            return;

        let  errMsg = rule['@error'] || rule['@errCode'],   // default error message when error occured
             parName = key  ?  (key + '.' + p) : p;
        ...
        if  (inObj[p] != undefined && (inObj[p] != '' || typeof inObj[p] != 'string'))  {
