...
function  requireBuild(id, params)  {
    ...
    let  nxBdCtx = bdCtx.childContext(uicPath, params);
    //xsPath += '.xs';
...
function  importBuild(uicName, id, params)  {
    ...
    let  xsPath = this.uicRoot + uicName,
         bdCtx = this.bdCtx.childContext(uicName, params);
    bdCtx.blockRoot = this.uicRoot;

    let  uic = readUIC(xsPath, this.uicRoot, bdCtx, true),
         node;
...
function  importUICFun(packPath)  {
    let  blockRoot = this.bdCtx.blockRoot,
         idx = blockRoot.indexOf(path.sep + 'themes' + path.sep),
...
function  parseJSONH(root, jh, inData, bdCtx, rowCount)  {
    ...
    if (names.length <= 1)  {
        ...
        if (server)  {
            bdCtx = bdCtx.childContext( url, inData );
