...
class JSONH {
    ...
    add(tag, attributes, children)  {
        ...
        if (this.isSingleton || (alen === 1 && !tag))
            // nothing to do!
            return  this;

        if (alen === 1 && Array.isArray(tag))  {
            tag.forEach( c => {
                if (c instanceof JSONH)
                    this.pushChild( c )
                else  if (Array.isArray(c) && (c = isArrayTag(c)))
                    this.pushChild( new JSONH(c[0], c[1], c[2]) )
            })
        }
        else  {
            if (alen === 2)  {
        ...
        if (this.uicList)  {
            this.uicList.forEach( uic => {
                ...
                if (uic.id)  {
                    ...
                    jsCode += 'let _' + uicID + '=__._' + uicID + '=' + uic.toJS(true) + 
...
function  makeAChild(c)  {
    if (c.length)  {
        let  ca = isArrayTag(c)
        return   ca  ?  new JSONH(ca[0], ca[1], ca[2]) : null
    }
    return  null
}


function  isArrayTag(a)  {
    const  alen = a.length
    let  a0 = a[0],
         tag

    if (typeof a0 === 'string' && alen <= 3)  {
        if (alen === 1)
            tag = [a0, null, null]
        else  if (alen === 2)  {
            let  a1 = a[1]

            if (typeof a1 === 'string' || a1 instanceof JSONH)
                tag = [a0, null, a1]
            else  if (Array.isArray(a1))
                tag = isArrayTag(a1)  ?  [a0, null, a1] : null
            else
                tag = [a0, a1, null]
        }
        else  if (alen === 3 && (typeof a[1] === 'object'))
            tag = a
    }
            ...
            if (key[0] === '@')
                ...
                if (cssValue)  {
                    let  selector = prefixSelector(prefix || '', key),
                         cssDeco = toCSSProperties( cssValue, leading + space, space )
                         
                    if (cssDeco !== '{}')
                        s += leading + selector + ' ' + cssDeco
                }
            }
        });
    }
